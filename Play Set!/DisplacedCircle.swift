...
struct DisplacedCircle: Shape {
    ...
    func path(in rect: CGRect) -> Path {
        ...
        let rightPoint = CGPoint(x: center.x + diameter * 0.75, y: center.y)
        let midRightPoint = CGPoint(x: center.x + diameter * 0.25, y: center.y)
        let midLeftPoint = CGPoint(x: center.x - diameter * 0.25, y: center.y)
